...
#define Z_INCREMENT 1.0             // mm per scan layer
...
// --- Functions ---
float readAveragedDistance(int samples = 10) {
  float sum = 0;
  int validSamples = 0;
...
void loop() {
  ...
  if (rotationStep < STEPS_PER_ROTATION) {
    if (!rotStepper.isRunning()) {
      // Take averaged distance reading
      float distance = readAveragedDistance(10); // 10 samples averaged

      if (distance == -1) {
        Serial.println("Sensor timeout or invalid readings!");
      } 
      else {
